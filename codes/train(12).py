    ...
    for direc in ['train', 'test', 'validate']:
        ...
        for dirName, subdirList, fileList in os.walk(PathDicom):
            ...
            if any(".dcm" in s for s in fileList):
                ...
                number = len(indice)
                imgs = np.zeros( [number, img_rows, img_cols])

                for filename in np.sort(list(fileList)):
...
def keras_fit_generator(img_rows=400, img_cols=400, n_imgs=10**4, batch_size=120, regenerate=True): ##True gaichengle false
...
start = time.time()
keras_fit_generator(img_rows=400, img_cols=400, regenerate=True,
                   n_imgs=15*10**4, batch_size=120)
